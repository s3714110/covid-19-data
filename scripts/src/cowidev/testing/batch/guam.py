...
class Guam(CountryTestBase):
    ...
    def pipe_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        pos = (
            df[(df["overall_outcome"] == "Positive") & (df["state_name"] == "Guam")]
        ...
        df["Positive rate"] = (
            (
                ...
                / (df["Daily change in cumulative total"].shift(-3).rolling(7).mean())
            )
            .round(3)
            .shift(3)
        )
