...
class UnitedStatesVirginIslands(CountryTestBase):
    ...
    def pipe_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        pos = (
            df[(df["overall_outcome"] == "Positive") & (df["state_name"] == "U.S. Virgin Islands")]
        ...
        df = (
            df[df["state_name"] == "U.S. Virgin Islands"]
            .groupby(["Date"], as_index=False)
        ...
        df["Positive rate"] = (
            (
                ...
                / (df["Daily change in cumulative total"].shift(-9).rolling(7).mean())
            )
            .round(3)
            .shift(9)
        )
