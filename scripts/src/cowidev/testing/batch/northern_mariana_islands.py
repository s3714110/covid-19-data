...
class NorthernMarianaIslands(CountryTestBase):
    ...
    def pipe_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        pos = (
            df[(df["overall_outcome"] == "Positive") & (df["state_name"] == "Northern Mariana Islands")]
        ...
        df = (
            df[df["state_name"] == "Northern Mariana Islands"]
            .groupby(["Date"], as_index=False)
        ...
        df["Positive rate"] = (
            (
                ...
                / (df["Daily change in cumulative total"].shift(-4).rolling(7).mean())
            )
            .round(3)
            .shift(4)
        )
